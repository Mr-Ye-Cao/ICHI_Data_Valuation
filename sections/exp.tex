...
\begin{figure*}[t]
    ...
    \caption{Gradient-based attribution validates domain relevance: biomedical content shows higher influence than entertainment. (A) Formatted data: fine-tuning and medical pretrain data consistently outperform entertainment pretrain across all epochs, demonstrating that biomedical content receives higher attribution scores. (B) Raw data: medical content maintains higher influence than entertainment even without QA formatting, confirming content-based rather than format-based attribution.}
...
\begin{figure*}[t]
    ...
    \caption{LARK achieves both storage efficiency and attribution accuracy advantages. (A) Storage efficiency: LARK requires only 223 MB for 500 samples compared to TracIn's 7.3 TB, achieving 32,768$\times$ compression through OPORP dimensionality reduction. (B) Attribution accuracy: LARK correctly identifies fine-tuning data as more influential in 86.6\% of queries, compared to only 40.4\% for BM25 (near random baseline of 50\%), demonstrating a 46.2 percentage point improvement over lexical methods.}
